  ...
  useEffect(() => {
    const fetchDances = async () => {
      try {
        ...
        if (!dbpediaResponse.ok)
          throw new Error("Failed to fetch DBpedia data");
        ...
        if (dbpediaResults.length > 0) {
          ...
          const formattedDances: Dance[] = dbpediaResults.map(
            (result: any) => ({
          ...
          setDances([
            {
              ...
              url: foundWikiData.thumbnail
                ? foundWikiData.thumbnail.source
                : "/placeholder.jpg",
            },
          ]);
          return;
        ...
        if (searchData.query.search.length > 0) {
          ...
          const pageSummaryEndpoint = `https://en.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(
            pageTitle
          )}`;
  ...
  return (
    <div className="container mx-auto p-6">
      <h1 className="text-3xl font-bold mb-4">
        Dances of {decodeURIComponent(country as string)}
      </h1>
